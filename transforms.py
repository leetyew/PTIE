# Taken from https://github.com/FangShancheng/ABINet
...
#             self.transforms = CVRandomPerspective(distortion=distortion)
        self.transforms = [CVRandomRotation(degrees=degrees), CVRandomAffine(degrees=degrees, translate=translate, scale=scale, shear=shear), CVRandomPerspective(distortion=distortion)]
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            if type_p < 0.33:
                return Image.fromarray(self.transforms[0](img))
            elif type_p < 0.66:
                return Image.fromarray(self.transforms[1](img))
            else:
                return Image.fromarray(self.transforms[2](img))
        else: return img
...
#         self.transforms =  transforms
        ...
        self.transforms = []
        if var is not None:
            self.transforms.append(CVGaussianNoise(var=var))
        if degrees is not None:
            self.transforms.append(CVMotionBlur(degrees=degrees))
        if factor is not None:
            self.transforms.append(CVRescale(factor=factor))
